...
def save_and_restart():
    ...
    write_theme(new)
    # Freeply.py'yi yeniden başlat
    freeply_path = os.path.join(BASE_DIR, "Freeply.py")
    subprocess.Popen([sys.executable, freeply_path])
    # Eski Freeply penceresini kapat (PID argüman olarak geldiyse)
    if len(sys.argv) > 1:
        try:
